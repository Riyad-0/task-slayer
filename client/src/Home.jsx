      ...
      <div className="home-monsters-section">
        <div className="home-monsters-container">
          ...
          <div className="home-monsters">
            {monsters.map(m => {
              return (
                <Monster key={m.id} monster={m} monsters={monsters} setMonsters={updateMonsters} />
              );
...
function MonsterView({ monster, monsters, setMonsters, switchToEdit }) {
  ...
  return (
    <div className="font-sans p-2 bg-slate-700 rounded-sm text-slate-100" key={monster.id}>
        {/* <div className="">
          <div className="flex flex-col gap-y-0.5 items-center mr-2 float-left ">
          ...
          <div className="wrap-break-word">{name}</div>
          <div className="wrap-break-word mt-2">{monster.task}</div>
        </div> */}
        <div className="flex gap-x-2">
...
function AttackButtonOrCompleted({ monster, attack }) {
  ...
  return (
    isTaskCompleted(monster) ?
      <div className="bg-green-600 h-7 rounded-[14px] w-full text-center" >Resting</div> :
  ...
  return (
    <div className="flex items-center gap-x-2">
      ...
      {/* <svg width="200" height="200" viewBox="0 0 100 100">
        <!-- M: Move to center (50,50) -->
        <!-- L: Line to start point (100,50) -->
        <!-- A: Arc (radius 50,50), no rotation, large-arc flag, sweep flag, end point -->
        <!-- Z: Close path back to center -->
        <path d="M 50 50 L 100 50 A 50 50 0 1 1 50 0 Z" fill="#3498db" />
      </svg> */}
    </div>
  );
}
